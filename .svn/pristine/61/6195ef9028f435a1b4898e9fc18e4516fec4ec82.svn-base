...
public class AudioPlayerUtil {
    ...
    private AudioPlayerUtil() {
        ...
        int musicInitLength = carlifeMusicInit.getSerializedSize();
        byte[] musicInitData = new byte[lenMsgHead + musicInitLength];
        System.arraycopy(ByteConvert.intToBytes(lenMsgHead + musicInitLength), 0, head, LEN_OF_FRAME_HEAD, LEN_OF_FRAME_HEAD);
        System.arraycopy(ByteConvert.intToBytes(musicInitLength), 0, musicInitData, 0, LEN_OF_FRAME_HEAD);
        System.arraycopy(ByteConvert.longToBytesLowbyte(System.currentTimeMillis()), 0, musicInitData, LEN_OF_FRAME_HEAD, LEN_OF_FRAME_HEAD);
        System.arraycopy(ByteConvert.intToBytes(CommonParams.MSG_MEDIA_INIT), 0, musicInitData, LEN_OF_MSG_HEAD, LEN_OF_FRAME_HEAD);
        System.arraycopy(carlifeMusicInit.toByteArray(), 0, musicInitData, lenMsgHead, musicInitLength);

        AOAAccessorySetup.getInstance().bulkTransferOut(head, LEN_OF_MSG_HEAD, musicInitData, musicInitData.length);
    }
