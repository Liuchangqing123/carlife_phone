 ...
 */
public class AndroidDebugConnect {

    private final String TAG = AndroidDebugConnect.class.getSimpleName();

    public static int ConnectType = ConnectManager.CONNECTED_BY_ANDROID_DEBUG;
    private static final String SOCKET_READ_THREAD_NAME = "SocketReadThread";

    private static AndroidDebugConnect mInstance = null;

    private final String UsbAndroidDebugPrefix = "UsbAndroidDebug_";

    private static final int LEN_OF_MSG_HEAD = 8;

    public static final int SOCKET_NAME_AndroidDebug_PORT = 7240;
    public static final int SOCKET_VIDEO_AndroidDebug_PORT = 8240;
    public static final int SOCKET_AUDIO_AndroidDebug_PORT = 9240;
    public static final int SOCKET_AUDIO_TTS_AndroidDebug_PORT = 9241;
    public static final int SOCKET_AUDIO_VR_AndroidDebug_PORT = 9242;
    public static final int SOCKET_TOUCH_AndroidDebug_PORT = 9340;
    ...
    public static AndroidDebugConnect getInstance() {
        if (null == mInstance) {
            synchronized (AndroidDebugConnect.class) {
                if (null == mInstance) {
                    mInstance = new AndroidDebugConnect();
                }
            }
        }
        return mInstance;
    ...
    public void startSocketReadThread() {
        try {
            mSocketReadThread = new SocketReadThread(SOCKET_NAME_AndroidDebug_PORT, UsbAndroidDebugPrefix + CommonParams.SERVER_SOCKET_NAME);
            mSocketReadThread.start();

            mSocketReadAudioThread = new SocketReadThread(SOCKET_AUDIO_AndroidDebug_PORT, UsbAndroidDebugPrefix + CommonParams.SERVER_SOCKET_AUDIO_NAME);
            mSocketReadAudioThread.start();

            mSocketReadAudioVRThread = new SocketReadThread(SOCKET_AUDIO_VR_AndroidDebug_PORT, UsbAndroidDebugPrefix + CommonParams.SERVER_SOCKET_AUDIO_VR_NAME);
            mSocketReadAudioVRThread.start();

            mSocketReadVideoThread = new SocketReadThread(SOCKET_VIDEO_AndroidDebug_PORT, UsbAndroidDebugPrefix + CommonParams.SERVER_SOCKET_VIDEO_NAME);
            mSocketReadVideoThread.start();

            mSocketReadTouchThread = new SocketReadThread(SOCKET_TOUCH_AndroidDebug_PORT, UsbAndroidDebugPrefix + CommonParams.SERVER_SOCKET_TOUCH_NAME);
            mSocketReadTouchThread.start();

            mSocketReadAudioTTSThread = new SocketReadThread(SOCKET_AUDIO_TTS_AndroidDebug_PORT, UsbAndroidDebugPrefix + CommonParams.SERVER_SOCKET_AUDIO_TTS_NAME);
        ...
        public void run() {
            ...
            try {
                if (null != mServerSocket && isRunning) {
                    ...
                    if (mSocketName.equals(UsbAndroidDebugPrefix + CommonParams.SERVER_SOCKET_NAME)) {
                        ConnectManager.getInstance().initConnectType(ConnectManager.CONNECTED_BY_ANDROID_DEBUG);
                        ConnectClient.getInstance().setIsConnecting(true);
                        ConnectManager.getInstance().startConnectSocket();
                    }
                }
            } catch (Exception e) {
            ...
            try {
                while (mSocket != null && isRunning) {
                    ...
                    if (mSocketName.equals(UsbAndroidDebugPrefix + CommonParams.SERVER_SOCKET_NAME)) {
                    ...
                    } else if (mSocketName.equals(UsbAndroidDebugPrefix + CommonParams.SERVER_SOCKET_TOUCH_NAME)) {
                        if (readData(msg, 0, CommonParams.MSG_CMD_HEAD_SIZE_BYTE) < 0) {
                            continue;
                        }
                        lenMsgHead = CommonParams.MSG_CMD_HEAD_SIZE_BYTE;
                    ...
                    } else if (mSocketName.equals(UsbAndroidDebugPrefix + CommonParams.SERVER_SOCKET_AUDIO_VR_NAME)) {
                        ...
                        DigitalTrans.dumpData(TAG, "RECV CarlifeVRMsg CMD", carlifeVRMessage, false);
