...
public class AESManager {
    ...
    public void init() {
        ...
        LogUtil.d("AESManager", "aesKey " + aesKey);
        if (aesKey == null) {
            return;
        }
    ...
    public byte[] decrypt(byte[] encryptData, int len) {
        // Avoid triggering init () actions asynchronously
        LogUtil.d("AESManager", "decrypt " + ByteConvert.printHexString(encryptData, encryptData.length));
        if (!ConnectClient.getInstance().isCarlifeConnected()) {
            return null;
        }
        if (!mIsInitDone) {
        ...
        try {
            ...
            e.printStackTrace();
            LogUtil.d("AESManager", "decrypt exception " + e.getMessage());
        } catch (BadPaddingException e) {
            e.printStackTrace();
            LogUtil.d("AESManager", "decrypt exception " + e.getMessage());
        }
