    ...
    private class DecoderThread extends Thread {
        ...
        public void run() {
            ...
            while (isRunning) {
                ...
                if (flag != 4) {
                    LogUtil.e(TAG, "decoderThread get data length failed " + flag);
                    break;
                }
                dataLength = ((inputData[0] << 24) & 0xff000000)
                        + ((inputData[1] << 16) & 0xff0000) + ((inputData[2] << 8) & 0xff00)
                        + ((inputData[3] << 0) & 0xff);
                LogUtil.d(TAG, "dataLength " + dataLength);
                ...
                if (dataLength == 0) {
                    ...
                    LogUtil.e(TAG, "empty package, used for heart beat");
                    continue;
                }
            ...
            try {
                while (isRunning) {
                    try {
                        ...
                        Thread.sleep(250);
                        LogUtil.e(TAG, "display exception " + e.getMessage());
                    }
                }
            } catch (InterruptedException e) {
                e.printStackTrace();
                LogUtil.e(TAG, "display exception InterruptedException " + e.getMessage());
            } catch (NullPointerException e) {
                // input thread is resetting the decoder, mDecoder is null,
                e.printStackTrace();
                LogUtil.e(TAG, "display exception NullPointerException " + e.getMessage());
                this.stopThread();
            }
        }
    }
